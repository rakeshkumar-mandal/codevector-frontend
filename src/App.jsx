...
export default function App() {
  ...
  const [cursorStack, setCursorStack] = useState([]); // history of cursors for back navigation
  ...
  const fetchId = useRef(0); // prevents stale responses from updating state
  ...
  const fetchProducts = useCallback(
    async (cursor, stack, catOverride) => {
      const category =
        catOverride !== undefined ? catOverride : selectedCategory;
  ...
  function handlePrev() {
    ...
    fetchProducts(prevCursor || null, newStack, undefined);
  }
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 16px" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: 14 }}>
          200,000 products · cursor-based pagination · consistent during live updates
        </p>
      </div>
      ...
      {!loading && products.length > 0 && (
        ...
        >
          <button
            ...
            style={btnStyle(cursorStack.length === 0)}
          >
            ← Previous
          </button>

          <span style={{ color: "var(--text-muted)", fontSize: 13 }}>
            Page {pageNum} · {products.length} results
          </span>

          <button
            onClick={handleNext}
            disabled={!nextCursor}
            style={btnStyle(!nextCursor)}
          >
            Next →
          </button>
        </div>
